...
@everywhere function mean_learning_Gaussian_Cat(diclist,box=1.0,rep=10)
    ...
    for i=1:rep
        Weightsto1,Weightsto2 = learning_poisson_input_unsupervised(simulations,box)
...
@everywhere function save_meanlearning_Gauss(diclist)
    ...
    fname = "sims\\09-script\\results\\$fnametemp"
    fnameparam = "sims\\09-script\\param\\"
...
@everywhere function generate_mean_weights_script09(simulation,indice)
...
@everywhere function save_accuracy_from_script09_batch(dicttemp,
	    ...
	    fname = "sims\\09-script\\P_App\\Batch=$indice\\$fnametemp" # P_App means performance_approx
