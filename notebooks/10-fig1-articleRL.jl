...
#%% Save example mean wights through time
using CSV
...
for i=1:100
	for j=1:dict["N"]
push!(df,[c.centers[j],i,mw1[i,j]])
#push!(df,[c.centers[j],i,mw2[i,j]])

	end
end

data = @dict mw1 mw2 dict
fname = "notebooks"
CSV.write(datadir(fname,"10-fig1-A.csv"),df)

#%ù weights with ocnfidence
dict = Dict(
    "N" => 25, # nombre de neurones dans le codage
    "T_simu" => 10000,
    "ΔTsave" => 100, # pas de temps de sauvegarde
...
clibrary(:colorcet)
plot(codinglayer.centers,mw1',color=:inferno_r,line_z=(1:100)')
plot!(legend=false)
savefig(plotsdir("04-Learning-using-confidence","Weights-Confidence-Opti.pdf"))


#<<<<<<<<<<


#%% Save example of decision
dict = Dict(
    "N" => 25, # nombre de neurones dans le codage
    "T_simu" => 10000,
    "ΔTsave" => 100, # pas de temps de sauvegarde
...
codinglayer.weightsto2 = mw2[end,:]
...
r1
r2
df = DataFrame(time = Float64[],r=Float64[],Unit = Int[])
for i=1:length(r1)
push!(df,[0.5*i,r1[i],1])
push!(df,[0.5*i,r2[i],2])
end
CSV.write(datadir(fname,"10-fig1-B.csv"),df)

r1temp = zeros(length(r1)-3)
r2temp = zeros(length(r1)-3)

for i=4:length(r1)
r1temp[i-3] = mean(r1[i-3:i])
r2temp[i-3] = mean(r2[i-3:i])
end

df = DataFrame(time = Float64[],r=Float64[],Unit = Int[])
for i=1:length(r1temp)
push!(df,[1.5+0.5*i,r1temp[i],1])
push!(df,[1.5+0.5*i,r2temp[i],2])
end
CSV.write(datadir(fname,"10-fig1-B.csv"),df)
